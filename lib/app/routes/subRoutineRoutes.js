    ...
    subRoutineRouter.route('/routines/:routine_id/subRoutines')
        .post((req, res, next) => {
            const
                ...
                subRoutine = SubRoutineManager.createSubRoutine(subRoutineModel, routineId, userId, next);
            res.json({
        ...
        .get((req, res, next) => {
            const
                userId = req.decoded.id,
                routineId = req.params.routine_id,
                subRoutines = SubRoutineManager.getSubRoutines(routineId, userId);
    ...
    subRoutineRouter.route('/routines/:routine_id/subRoutines/:subRoutine_id')
        .get((req, res, next) => {
            const
                ...
                subRoutine = SubRoutineManager.getSubRoutine(subRoutineId, next);
            res.json(subRoutine);
        })
        .put((req, res, next) => {
            const
