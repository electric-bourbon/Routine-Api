...
function UserRoutes(app, express) {
    ...
    userRouter.route('/users')
        .post((req, res, next) => {
            ...
            UserManager.createUser(userModel, next)
                .then((user) => {
                    res.json({
    ...
    userRouter.route('/users/:user_id')
        .get((req, res, next) => {
            ...
            UserManager.getUser(userId, next)
                .then((day) => {
                    res.json(user);
                });
        })
        .put((req, res, next) => {
            const
                ...
                userModel = req.body;
            UserManager.updateUser(userId, userModel, next)
                .then(() => {
                    res.json({
        ...
        .delete((req, res, next) => {
            const userId = req.params.user_id;
            DayManager.deleteAllDaysForUser(userId, next)
                .then(() => {
