...
function RoutineRoutes(app, express) {
    ...
    routineRouter.route('/routines')
        ...
        .get((req, res, next) => {
            const
                userId = req.decoded.id;
            RoutineManager.getRoutines(userId, next).then((routines) => {
                res.json(routines);
            });
        });
    ...
    routineRouter.route('/routines/:routine_id')
        .get((req, res, next) => {
            ...
            RoutineManager.getRoutine(routineId, next).then((routine) => {
                res.json(routine);
            });
        })
        .put((req, res, next) => {
            const
                ...
                routineModel = req.body;
            RoutineManager.updateRoutine(routineId, routineModel, next).then((routine) => {
                res.json({
        ...
        .delete((req, res, next) => {
            const routineId = req.params.routine_id;
            RoutineManager.deleteRoutine(routineId, next).then(() =>{
                    res.json({
